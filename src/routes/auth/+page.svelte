<script>
	...
	import '../../../static/themes/auth-page.css';
	...
	{#if loaded}
		<div class="fixed m-10 z-50">
			<div class="flex space-x-2">
				<div class="self-center auth-logo">
					<img
						id="logo"
			...
			<div class="w-full sm:max-w-md px-6 flex flex-col text-center">
				{#if ($config?.features.auth_trusted_header ?? false) || $config?.features.auth === false}
					<div class="my-auto pb-10 w-full">
						<div
							class="flex items-center justify-center gap-3 text-xl sm:text-2xl text-center font-semibold text-white auth-title"
						>
							<div>
					...
					<div class="w-full text-white auth-form-container">
						...
						>
							<div class="mb-3">
								...
								{#if $config?.onboarding ?? false}
									<div class="mt-1 text-xs font-medium auth-subtitle">
										ⓘ {$WEBUI_NAME}
										{$i18n.t(
							...
							{#if $config?.features.enable_login_form || $config?.features.enable_ldap}
								<div class="flex flex-col mt-4">
									{#if mode === 'signup'}
										<div class="mb-3">
											<div class="text-sm font-medium text-left mb-1 auth-label">{$i18n.t('Name')}</div>
											<input
												bind:value={name}
												type="text"
												class="my-0.5 w-full text-sm outline-hidden auth-input"
												autocomplete="name"
									...
									{#if mode === 'ldap'}
										<div class="mb-3">
											<div class="text-sm font-medium text-left mb-1 auth-label">{$i18n.t('Username')}</div>
											<input
									...
									{:else}
										<div class="mb-3">
											<div class="text-sm font-medium text-left mb-1 auth-label">{$i18n.t('Email')}</div>
											<input
												bind:value={email}
												type="email"
												class="my-0.5 w-full text-sm outline-hidden auth-input"
									...
									<div class="mb-2">
										<div class="text-sm font-medium text-left mb-1 auth-label">{$i18n.t('Password')}</div>
									...
									{:else}
										...
										{#if $config?.features.enable_signup && !($config?.onboarding ?? false)}
											<div class="mt-4 text-sm text-center">
												...
												<button
													class="font-medium underline auth-text-link"
													type="button"
													on:click={() => {
														if (mode === 'signin') {
															mode = 'signup';
						...
						{#if $config?.features.enable_ldap && $config?.features.enable_login_form}
							<div class="mt-2">
								<button
									class="flex justify-center items-center text-xs w-full text-center underline auth-text-link"
									type="button"
									on:click={() => {
										if (mode === 'ldap')
